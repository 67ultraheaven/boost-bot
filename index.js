...
const SERVER2_ID = '1488203493633294471'; // boosted server
const SERVER1_ID = '1472361414474727434'; // reward server
const BOOSTER_ROLE_ID = '1488206021796298774'; // booster role in server 2
const REWARD_ROLE_ID = '1488350841353277542'; // reward role in server 1
const TOKEN = process.env.TOKEN; // reset token first, then paste new one here
...
async function syncBoosters() {
  try {
    ...
    const sourceBoosterMembers = sourceGuild.members.cache.filter(member =>
    ...
    for (const [userId, sourceMember] of sourceBoosterMembers) {
      ...
      if (!targetMember.roles.cache.has(REWARD_ROLE_ID)) {
        ...
        console.log(`Synced role to ${sourceMember.user.tag}`);
        added++;
      }
    }
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.guildId !== SERVER1_ID) {
    await interaction.reply({
      content: 'Use this command in Server 1 only.',
      ephemeral: true
    });
    return;
  }
  ...
  if (!result) {
    await interaction.editReply('Sync failed. Check the console.');
    return;
  }
